...
3) Train-Models : This script will train the Slice Selection/Prediction Model and SM-training script will train the Segmentation Model. The trained models are copied into the model/test folder.

4) Test-Pipeline:  This script will run the Automated pipeline on the test cohort of CT scans. Raw scans are the input and the auto-segmentations are the output. 

5) Analysis-post-testing : This script will generate the evaluation metrics for the auto segmentations generated by automated pipeline coampred to ground truth.
