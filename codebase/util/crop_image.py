...
def crop_top(patient_id, img, seg, crop_shape, return_type, output_img_dir, 
    ...
    print(" in crop image spacing")
    print(img_sitk.GetSpacing())
    resizeShape =(256,256,img_sitk.GetSize()[2])
    if( img_sitk.GetSize()[0] < 256) :
        img_sitk = resize_3d(img_sitk, sitk.sitkLinear, resizeShape)
    ...
    if(seg_sitk.GetSize()[0] < 256) :
        seg_sitk = resize_3d(seg_sitk, sitk.sitkLinear, resizeShape)
...
def crop_top_image_only1(patient_id, img_dir, crop_shape, return_type, output_dir, image_format):
    """
...
#    image = sitk.ReadImage(nrrd_image)
    ...
    print("printing input_spacing in the resize")
    print(input_spacing)
    output_spacing = (
