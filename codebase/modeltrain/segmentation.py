...
from util.data_generator import train_generator

from modeltrain.unet import get_unet_2D
from modeltrain.losses import dice_coef_multiclass_loss_2D
from util.callbacks import MultiGPUModelCheckpoint
from util.generators import SegmentationSequence



def train(data_dir, model_dir, name=None, epochs=200, batch_size=1, load_weights=None,
    ...
    print("compression channels")
    print(compression_channels)
    decompression_channels=sorted(compression_channels,reverse=True)[1:]
    print("decompression channels")
    print(decompression_channels)
